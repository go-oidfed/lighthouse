...
!!! info
    ...
    You must be at least on version 0.6.0 to migrate to 0.20.X.
...
| `federation_data.trust_anchors` | `trust_anchors` | Trust anchors with JWKS, refreshing settings |
| `endpoints.auth.trust_anchors` | `trust_anchors` | Default auth trust anchors (merged into TA repository) |
| `endpoints.*.path/url/auth_*` | `endpoints` | Federation endpoint paths, URLs, auth settings |
| `endpoints.resolve.*` | `endpoints` | Resolve settings (grace period, proactive resolver, allowed TAs) |
| `endpoints.enroll.checker` | `endpoints` | Enroll entity checker config (stored as JSON, TAs as entity-id refs) |
| `endpoints.entity_collection.*` | `endpoints` | Entity collection settings (interval, concurrency, pagination) |
...
| `storage.endpoint_auth.jti_backend` | JTI storage backend ("cache" or "db") |
| `storage.endpoint_auth.jti_cleanup_interval` | JTI cleanup interval (DB backend only) |
...
!!! note "federation_data section deprecated"
    ...
    [Federation Data](../config/db/entity-configuration.md) for migration details.
...
!!! info "Trust Anchors & Endpoints Migration"

    A detailed migration guide for the trust anchor repository and dynamic
    federation endpoints is available at
    [Migration: TA Repository & Dynamic Endpoints](0.22.md).
...
- `trust_anchors` - Trust anchors (federation_data.trust_anchors + endpoints auth TAs)
- `endpoints` - Federation endpoint paths/config (endpoints.*)
