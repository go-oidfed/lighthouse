...
func TestGetTimeSeries_SqliteHourBuckets(t *testing.T) {
	...
	logs := []*stats.RequestLog{
		...
		{
			Timestamp: base.Add(1 * time.Hour).Add(2 * time.Minute),
			Endpoint: "fetch",
			Method: "GET",
			StatusCode: 200,
			DurationMs: 40,
...
func TestGetTimeSeries_SqliteDayBuckets(t *testing.T) {
	...
	logs := []*stats.RequestLog{
		...
		{
			Timestamp: day1.Add(3 * time.Hour),
			Endpoint: "resolve",
			Method: "GET",
			StatusCode: 404,
			DurationMs: 7,
...
func TestGetDailyStats_SqliteRange(t *testing.T) {
	...
	rows := []stats.DailyStats{
		{
			Date: midnight(2026, 7, 19),
			Endpoint: "fetch",
			StatusCode: 200,
			RequestCount: 2814,
		},
		{
			Date: midnight(2026, 7, 20),
			Endpoint: "fetch",
			StatusCode: 200,
			RequestCount: 4830,
		},
		{
			Date: midnight(2026, 7, 23),
			Endpoint: "fetch",
			StatusCode: 500,
			RequestCount: 12,
			ErrorCount: 12,
	...
	for _, iv := range intervals {
		t.Run(
			string(iv), func(t *testing.T) {
				...
				assert.True(
					t, points[0].Timestamp.Before(ts.Add(time.Second)) || points[0].Timestamp.Equal(ts),
